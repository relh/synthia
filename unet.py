...
class UNet(nn.Module):
    def __init__(self, in_channels, out_im_channels, batchnorm, dropout=0.3, regression=False, bins=80, bc=64):
        ...
        self.inc = inconv(in_channels, bc*1+(0 if regression else 32), batchnorm)
        self.down1 = down(bc*1+(0 if regression else 32), bc*2, batchnorm, dropout=dropout)
        ...
        self.up3 = up(bc*4, bc*1+(0 if regression else 32), batchnorm, dropout=dropout)
        self.up4 = up(bc*2+(0 if regression else 64), bc*2+(0 if regression else 64), batchnorm, dropout=dropout)
        self.outc = outconv(bc*2+(0 if regression else 64), out_im_channels, regression, bins)
...
def convrelu(in_channels, out_channels, kernel, padding):
    return nn.Sequential(
      nn.Conv2d(in_channels, out_channels, kernel, padding=padding, padding_mode='reflect'),
    ...
    def __init__(self, in_ch, out_ch, batchnorm=True):
        ...
        if batchnorm:
            self.conv = nn.Sequential(
              ...
              nn.Conv2d(out_ch, out_ch, 3, padding=1, padding_mode='reflect'), nn.BatchNorm2d(out_ch), nn.ReLU(inplace=True))
        else:
            self.conv = nn.Sequential(
              nn.Conv2d(in_ch, out_ch, 3, padding=1, padding_mode='reflect'), nn.ReLU(inplace=True),
              nn.Conv2d(out_ch, out_ch, 3, padding=1, padding_mode='reflect'), nn.ReLU(inplace=True))
